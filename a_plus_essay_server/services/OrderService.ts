...
export class OrderService {
  ...
  async matchTutor(orderId: number) {
    try {
      ...
      if (tutor5Id) {
        matchedTutors.push({ id: tutor5Id });
      ...
      const tutor4Id = (
        await this.knex
          ...
          .innerJoin("tutor", "preferred_subject.tutor_id", "=", "tutor.id")
          .whereBetween("rating", [4.0, 4.99])
          .orderBy("ongoing_order_amount", "asc")
          .first()
      ).id;
      if (tutor4Id) {
        matchedTutors.push({ id: tutor4Id });
      } else matchedTutors.push({ id: -1 });
      ...
      const tutor3Id = (
        await this.knex
          ...
          .innerJoin("tutor", "preferred_subject.tutor_id", "=", "tutor.id")
          .whereBetween("rating", [3.0, 3.99])
          .orderBy("ongoing_order_amount", "asc")
          .first()
      ).id;
      if (tutor3Id) {
        matchedTutors.push({ id: tutor3Id });
      } else matchedTutors.push({ id: -1 });
      ...
      if (newTutorId) {
        matchedTutors.push({ id: newTutorId });
      } else matchedTutors.push({ id: -1 });
      ...
      matchedTutors.map((tutor) => {
        if (tutor.id > 0) {
          this.io.to(`${tutor.id}`).emit("new-order", "You have an new order.");
        }
      });
    } catch (error) {
      console.log(error);
  ...
  async rejectQuotation(input: { orderId: number; tutorId: number }) {
    ...
    const tutorIdList = await this.knex
      ...
      .from("candidate")
      .where("order_id", input.orderId);
    const category: number = await this.knex
      .select("category")
      .from("candidate")
      .where("order_id", input.orderId)
      .andWhere("tutor_id", input.tutorId)
      ...
      switch (category) {
        case 5:
          ...
          found = tutorIdList.find(
            (tutorId) => tutorId !== -1 && tutorId === tutor5Id
          );
          if (found === undefined) {
        ...
        case 4:
          ...
          found = tutorIdList.find(
            (tutorId) => tutorId !== -1 && tutorId === tutor4Id
          );
          if (found === undefined) {
            tutorHasBeenSelected = false;
            await this.knex
        ...
        case 3:
          ...
          found = tutorIdList.find(
            (tutorId) => tutorId !== -1 && tutorId === tutor3Id
          );
          if (found === undefined) {
            tutorHasBeenSelected = false;
            await this.knex
