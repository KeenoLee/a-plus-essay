...
export async function seed(knex: Knex): Promise<void> {
    ...
    const row =
        await knex("user").insert([
            { is_admin: false, is_tutor: false, nickname: 'admin', email: 'admin', hashed_password: await hashPassword('admin'), phone_number: 12345678 },
            { is_admin: false, is_tutor: true, nickname: 'tutor', email: 'tutor', hashed_password: await hashPassword('tutor'), phone_number: 12345679 },
            { is_admin: true, is_tutor: false, nickname: 'student', email: 'student', hashed_password: await hashPassword('student'), phone_number: 12345688 }
        ]).returning("id");
